<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Registrar cliente y vehículo') }}
        </h2>
    </x-slot>
    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-5xl mx-auto">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-2xl sm:text-3xl font-bold text-gray-800">Agregar registro</h2>
            ...
            <form id="cliente-soat-form" method="POST" action="{{ route('clientes.store') }}" class="bg-white rounded-lg shadow-lg p-6 sm:p-8">
                @csrf
                ...
                <div class="mb-8 p-5 rounded-xl border-2 border-dashed border-blue-300 bg-blue-50/60">
                    ...
                    <p class="text-sm text-gray-600 mb-4 leading-relaxed">
                        Pega aquí un bloque copiado (correo, Excel, cotización, etc.) con líneas tipo <strong>Etiqueta: valor</strong>.
                        Luego pulsa el botón: se rellenarán los campos del formulario; <strong>no se guarda</strong> nada hasta que des «Guardar».
                    </p>
                    ...
                    <textarea
                        ...
                        placeholder="Placa&#10;AAS91H&#10;Linea&#10;180S&#10;Marca&#10;BENELLI&#10;… (etiqueta y valor en líneas seguidas). También acepta Placa: ABC123 en una sola línea."
                    ></textarea>
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="text" name="placa" value="{{ old('placa') }}" required maxlength="10" data-upper
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Línea</label>
                            <input type="text" name="linea" value="{{ old('linea') }}" placeholder="Ej. 180S"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Marca</label>
                            <input type="text" name="marca" value="{{ old('marca') }}" placeholder="Ej. BENELLI"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Modelo (año)</label>
                            <input type="number" name="año" value="{{ old('año') }}" min="1900" max="{{ date('Y') + 1 }}" placeholder="Ej. 2023"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Clase</label>
                            <input type="text" name="clase" value="{{ old('clase') }}" placeholder="Ej. MOTOCICLETA"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        <div class="md:col-span-2">
                            ...
                            <select name="tipo_vehiculo" required
                                ...
                                @foreach (['MOTOCICLETA' => 'Motocicleta', 'AUTOMOVIL' => 'Automóvil', 'CAMPERO' => 'Campero', 'CAMIONETA' => 'Camioneta', 'CARGA' => 'Carga', 'MOTOCARRO' => 'Motocarro', 'SERVICIO_PUBLICO' => 'Servicio público', 'OTRO' => 'Otro'] as $val => $label)
                                    <option value="{{ $val }}" {{ old('tipo_vehiculo') === $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="tipo_documento" required
                                ...
                                <option value="CE" {{ old('tipo_documento', 'CC') === 'CE' ? 'selected' : '' }}>Cédula de extranjería</option>
                                <option value="PA" {{ old('tipo_documento', 'CC') === 'PA' ? 'selected' : '' }}>Pasaporte</option>
                                <option value="NIT" {{ old('tipo_documento', 'CC') === 'NIT' ? 'selected' : '' }}>NIT</option>
                            </select>
                        </div>
                        <div>
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" name="valor" value="{{ old('valor') }}" required min="0" step="0.01"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                        <div>
                            ...
                            <select name="estado" required
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                <option value="vigente" {{ old('estado', 'vencida') === 'vigente' ? 'selected' : '' }}>Vigente</option>
                                <option value="vencida" {{ old('estado', 'vencida') === 'vencida' ? 'selected' : '' }}>Vencida</option>
                                <option value="proxima_vencer" {{ old('estado', 'vencida') === 'proxima_vencer' ? 'selected' : '' }}>Próxima a vencer</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap justify-end gap-4 pt-2">
                    ...
                    <button type="submit" class="px-8 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition font-medium shadow-md">Guardar</button>
                </div>
            </form>
        </div>
    </div>
