...
/* harmony import */ var _soat_cliente_autofill__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./soat-cliente-autofill */ "./resources/js/soat-cliente-autofill.js");
/* harmony import */ var _soat_cliente_autofill__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_soat_cliente_autofill__WEBPACK_IMPORTED_MODULE_1__);
...
/***/ "./resources/js/soat-cliente-autofill.js"
/*!***********************************************!*\
  !*** ./resources/js/soat-cliente-autofill.js ***!
  \***********************************************/
() {
...
function normalizeKey(s) {
  ...
  out = stripAccents(out).toLowerCase().replace(/^\*+\s*/, '').replace(/\s+/g, ' ').trim();
...
function mapTipoDocumento(v) {
  var x = stripAccents(String(v).toUpperCase()).replace(/\s+/g, ' ').trim();
  if (!x) return '';
...
function mapTipoVehiculo(v) {
  var x = stripAccents(v).toUpperCase().replace(/\s+/g, ' ').trim().replace(/_/g, ' ');
  var keys = ['MOTOCICLETA', 'AUTOMOVIL', 'CAMPERO', 'CAMIONETA', 'CARGA', 'MOTOCARRO', 'SERVICIO_PUBLICO', 'OTRO'];
  for (var k = 0; k < keys.length; k += 1) {
    var key = keys[k];
    var keySpaced = key.replace(/_/g, ' ');
...
function standaloneLabelToField(line) {
  ...
  if ((n.includes('numero') || n.includes('nro')) && n.includes('documento') && !n.includes('tipo')) {
...
function parseTwoLinePairs(text, pairs) {
  ...
  for (var i = 0; i < lines.length; i += 1) {
    ...
    var field = standaloneLabelToField(line);
    if (!field) continue;
    var j = i + 1;
...
function labelToField(label) {
  var n = normalizeKey(label);
  var nAscii = stripAccents(n);
  ...
  var docNumLabel = (n.includes('documento') || n.includes('identificaci') || n.includes('cedula')) && !n.includes('vehiculo') && !n.includes('tipo');
  if (docNumLabel) {
    if (n.includes('numero') || n.includes('nro') || n.includes('nº') || n.includes('n°') || /^no\.?\s/.test(n) || n.startsWith('no ') || n === 'identificacion' || n.endsWith(' identificacion')) {
      return 'numero_documento';
    }
  }
  ...
  if (n === 'cedula' || n === 'c.c.' || n === 'c.c' || n === 'cc' || n.startsWith('cedula ') && !n.includes('extranjer')) {
    return 'numero_documento';
  }
...
function extractDocumentPairsFromText(text, pairs) {
  ...
  if (!pairs.numero_documento || !pairs.tipo_documento) {
    var re = /\b(CC|CE|PA|NIT)\b\s*[:\-]?\s*([\d.]{5,20})\b/gi;
    var m = re.exec(t);
    while (m) {
      var tipo = m[1].toUpperCase();
      var rawNum = m[2].replace(/\./g, '');
  ...
  if (!pairs.numero_documento) {
    var m2 = t.match(/(?:cedula|identificaci[oó]n|c\.?\s*c\.?)\s*(?:no\.?|n[°º]\s*|n[uú]mero\s*)?[.:]?\s*([\d.\s]{6,20}\d)/i);
    if (m2) {
      var raw = m2[1].replace(/[^\d]/g, '');
...
function setSelectByValue(select, rawValue, name) {
  ...
  var rawTipoDoc = name === 'tipo_documento' ? code : '';
  if (name === 'tipo_vehiculo') {
    code = mapTipoVehiculo(code);
  }
  if (name === 'tipo_documento') {
    var m = mapTipoDocumento(code);
    if (m) code = m;
  }
  var opts = Array.from(select.options);
  var codeNorm = stripAccents(code).toUpperCase();
  var hit = opts.find(function (o) {
    return o.value === code;
  });
  if (!hit) hit = opts.find(function (o) {
    return stripAccents(o.value).toUpperCase() === codeNorm;
  });
  if (!hit) {
    hit = opts.find(function (o) {
      return stripAccents(o.text).toUpperCase().replace(/\s+/g, ' ') === codeNorm.replace(/_/g, ' ');
    });
  }
  if (!hit && name === 'tipo_vehiculo') {
    hit = opts.find(function (o) {
      return mapTipoVehiculo(o.text) === code;
    });
  }
  if (!hit && name === 'tipo_documento') {
    var want = mapTipoDocumento(rawTipoDoc) || code;
    ...
    if (!hit && want) {
      hit = opts.find(function (o) {
        return mapTipoDocumento(o.text) === want;
      });
    }
  }
  if (hit && hit.value !== '') {
...
function procesarTextoSoatRegistro(form, textarea) {
  var f = form || document.getElementById('cliente-soat-form');
  var ta = textarea || document.getElementById('soat-paste-bloque');
  if (!f || !ta) {
  ...
  var linesArr = splitLines(raw);
  var text = linesArr.join('\n');
  var pairs = {};
  parseTwoLinePairs(text, pairs);
  var lineRe = /^([^:|\t]+)[:|\t]\s*(.+)$/;
  linesArr.forEach(function (line) {
    var m = line.trim().match(lineRe);
    if (m) {
      var field = labelToField(m[1]);
      if (field) {
  ...
  var placaLoose = text.match(/\bPlaca\s*[:#]?\s*([A-Z0-9]{5,10})\b/i);
  ...
  Object.keys(pairs).forEach(function (key) {
    if (key === '_modelo') {
      var val = pairs._modelo;
      if (/^\d{4}$/.test(val)) {
        setField(f, 'año', val);
      } else {
        var lineaEl = f.querySelector('[name="linea"]');
    ...
    if (key === 'estado') {
      var e = mapEstado(pairs[key]);
      if (e) setField(f, 'estado', e);
      return;
    }
    if (key === 'fecha_inicio' || key === 'fecha_fin') {
      var d = parseDateToHtmlInput(pairs[key]);
      if (d) setField(f, key, d);
      return;
    }
    if (key === 'valor') {
      var v = parseMoney(pairs[key]);
  ...
  var tipoDocEl = f.querySelector('[name="tipo_documento"]');
  if (tipoDocEl && !String(tipoDocEl.value || '').trim()) {
    setField(f, 'tipo_documento', 'CC');
  }
  var tipoVehSel = f.querySelector('[name="tipo_vehiculo"]');
